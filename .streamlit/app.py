...
from config import INDEX_NAME, PINECONE_API_KEY, NEBIUS_API_KEY, SYSTEM_PROMPT
...
def process_pdf(file_path):
    ...
    for i, chunk in enumerate(chunks):
        ...
        status = index.upsert(vectors=[{"id":str(i), "values":embedding, "metadata":{"text":content}}])
        if status["upserted_count"]>=1:
            st.markdown(f":white_check_mark: Chunk {i+1} upserted")
        else:
...
def query_rag_system(query_text,MSGS = SYSTEM_PROMPT):
    """
...
with st.sidebar:
    ...
    choice = st.radio("Navigation",["Upload","Chat"])
